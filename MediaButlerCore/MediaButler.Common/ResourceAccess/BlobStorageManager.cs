...
namespace MediaButler.Common.ResourceAccess
{
    ...
    /// MBF blob manager
    /// </summary>
    internal class BlobStorageManager : IButlerStorageManager
    {
        ...
        /// <summary>
        /// Read MBF configuration 
        /// </summary>
        /// <param name="partition">config partition key</param>
        /// <param name="row">config row key</param>
        /// <returns>string valur, empty if is not exist</returns>
        public string GetButlerConfigurationValue(string partition, string row)
        {
            string config = "";
            try
            {
                config = Configuration.MBFConfiguration.GetConfigurationValue(row, partition, storageConn);
            }
            catch (Exception)
            {
